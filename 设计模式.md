...
public class Main {
    public static void main(String[] args){
        ...
        Player playerProxy = (Player) Proxy.newProxyInstance(
                PlayerImpl.class.getClassLoader(), // 获取目标对象的类加载器
                PlayerImpl.class.getInterfaces(),  // 获取目标对象所有的接口
                new DynamicProxy(player)  // 创建处理器对象
        );
        // 调用被代理类的方法
        player.play();
        ...
        // 创建调用处理器对象
        DynamicProxy ListenHandler = new DynamicProxy(listener);
        // 动态生成代理对象
