...
简单工厂模式中 工厂 负责的是生产所有产品，而工厂方法模式将生成不同产品的任务分发给不同的产品工厂，正所谓各司其职，减少依赖性。
...
```java
public class PhoneFactory {
    public Object makePhone(AbstractFactory f) {
        return f.makePhone();
    }
}
```
...
public class Demo {
    public static void main(String[] arg) {
        PhoneFactory factory = new PhoneFactory(new MiFactory());
        factory.makePhone();	// 制造小米手机!
    }
}
```
