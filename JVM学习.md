...
5. 初始化：给类的静态变量赋初始值。如果有静态代码块就执行static代码块中定义的所有操作。如果父类没有初始化则**优先**对父类进行初始化。
...
- 用于存储基本类型数据（方法参数和方法内的局部变量）、对象引用。
- 局部变量表的容量以 变量槽 为最小单位，每个变量槽占用长度由具体虚拟机决定。基本类型数据、对象引用和返回地址占用一个变量槽，long 和 double 需要两个。
...
HotSpot中采用 `OopMap`  (Ordinary object pointer Map) 这个数据结构来**记录栈中引用数据类型的位置**，这样GC的时候只需要扫描Oop Mao中的数据即可，而无需对整个栈上内存进行扫描。
