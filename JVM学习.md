...
如果更新失败，就意味着至少存在一条线程与当前线程竞争获取该对象的锁。此时，虚拟机会检查锁对象的MarkWord是否指向当前线程的栈帧。如果是，则直接进入同步代码块。否则升级为重量级锁。

该线程会不断的进行CAS操作去尝试获取锁，当CAS操作达到一定数量的时候会升级为重量级锁。

在不断进行CAS的时候会比较消耗CPU资源，所以CAS操作不能进行太多次。因此，java也对
