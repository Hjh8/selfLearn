...
2. ps命令查看该进程下的各个线程占用cpu情况
   - `ps H -eo pid,tid,%cpu | grep pid`  -eo 后面跟要显示的信息；grep pid是过滤其他进程
...
- jmap：查看堆内存占用情况。一般使用 `jmap -heap 进程id` 命令查看对象进程的堆内存使用情况
...
方法区只是个**概念**，具体的实现还需要根据每个厂商来决定。在1.8以前，**永久代**是方法区的实现，存在于jvm的内存结构中。1.8以后移除了永久代，取代它的是**元空间**。元空间存在于本地内存中，并且将运行时常量池中的字符串常量池部分从元空间移到了堆中。
...
当某个类被加载时，它的常量池信息就会放到运行时常量池中。图中的常量池就是运行时常量池。
...
**引用技术法**：每个对象都有一个计数属性，若此对象被其他变量引用一次，计数属性就加一。不再引用则减一。为0时表示该对象为可回收对象。这种方式有个弊端，就是a引用b，b引用a，这时就会出现内存泄露。
...
**Minor GC触发机制**：
当Eden区满时就会触发Minor GC。注意：只有Survivor内存不足时并**不会**引发GC。



**对象进入老年代的四种情况：** 

1. 若进行Minor GC时发现存活的对象在To区放不下，则把存活的对象存入老年代
2. 如果在From空间中，所有相同年龄的对象的大小总和大于Survivor空间的一半，那么大于等于该年龄的对象就会被移动到老年代
3. 大对象直接进入老年代：刚创建的对象的内存大于Eden区时，并且老年代内存空间足够的情况下会直接晋升到老年代。
4. 长期存活的对象会进入老年代
...
吞吐量优先垃圾回收是多线程版本的串行回收器，即可以进行并行收集。
...
JVM内存结构指的是JVM的内存分区；而JVM内存模型是一种虚拟机规范。

JMM定义了一套在多线程读写共享数据时，对数据的可见性、有序性和原子性的规则。其作用也是用来屏蔽掉不同操作系统中的内存差异性来保持并发的一致性。
...
CPU中运行的线程从主存中拷贝共享数据到它的工作内存，并在之后对这个变量在本地内存中做出了更改，但这个变更对运行在其他CPU中的线程是不可见的，因为这个更改还没有更新到主存中。要解决这个可见性问题，我们可以使用**volatile**关键字。
...
- **read（读取）**：作用于主内存变量，把一个变量值从主内存传输到线程的工作内存中，等待load的执行。
- **load（载入）**：作用于工作内存的变量，把read操作得到的变量值载入工作内存的变量副本中。
- **use（使用）**：作用于工作内存的变量，把工作内存中的一个变量值传递给执行引擎。
- **assign（赋值）**：作用于工作内存的变量，把一个从执行引擎返回到的值赋值给工作内存的变量。
- **store（存储）**：作用于工作内存的变量，把工作内存中的一个变量的值传送到主内存中，等待write的执行。
- **write（写入）**：作用于主内存的变量，把store操作得到的变量值写入到主内存的变量中。

![image-20210619112640377](JVM学习.assets/image-20210619112640377.png)

总线中有个`MESI缓存一致协议` ：当某个CPU修改了共享副本里的数据，该数据会马上同步到主内存，然后使其他cpu副本中的该变量失效，重新去内存读取数据。

而volalite就是告诉总线**开启MESI缓存一致协议**，从而使变量可以即使的同步，实际就是能够即使的store跟load。store跟load的过程会加锁。
