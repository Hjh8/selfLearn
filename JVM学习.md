...
CPU中运行的线程从主存中拷贝共享数据到它的工作内存，并在之后对这个变量在本地内存中做了修改，但这个修改对运行在其他CPU中的线程是不可见的，因为这个修改还没有更新到主存中。要解决这个可见性问题，我们可以使用**volatile**关键字。
...
总线中有个`MESI缓存一致协议` ：当某个CPU修改了共享副本里的数据同步回主内存时，会通知其他线程共享变量副本中的该变量失效，重新去内存读取数据。

而volalite就是让总线**开启MESI缓存一致协议**，从而使变量可以及时的同步，实际就是变量一旦被修改就立马store跟load。
...
1. 句柄：Java 堆中将会划分出一块内存来作为句柄池，此时指针存储的就是对象的句柄地址，而句柄中包含了对象实例数据与类型数据的具体地址信息

   gc的时候，对象的地址改变时，指针的指向不用改变。

2. 直接指针： 指针存储的直接就是对象的地址。
