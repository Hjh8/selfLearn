...
> spring.main.web-application-type 表示 spring项目的类型，有**三种**：none（java项目）、servlet（web项目）、eactive（react项目）
...
starter相当于一个**jar包，里面设置了一些默认的配置信息**，需要使用时直接在maven中引入该starter坐标即可，springboot会自动的将该配置类中注册的bean放到IOC容器中。如果一个自定义配置类经常需要在别的项目中使用，那么可以将该自定义配置类来封装成一个starter，然后其他项目中使用的话直接引入坐标即可，这样可以很好的提高复用性。
