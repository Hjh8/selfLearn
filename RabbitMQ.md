...
**生产者生产的消息不会直接发送到队列**，而是将消息发送到交换机(exchange)。交换机工作的内容非常简单，一方面它接收来自生产者的消息，另一方面将它们推入队列。交换机必须确切知道如何处理收到的消息，即把这些消息放到特定队列还是说放到许多队列中还是说丢弃它们。
...
- 主题(topic) 
- 标题(headers) 
- 扇出(fanout)，发布订阅模式使用的交换机为**fanout** 
