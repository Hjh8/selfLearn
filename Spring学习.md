...
spring会将一个方法中所有的通知经过拓扑排序后加入到chain对象（拦截器链）中，chain对象是一个List，第一个元素固定是**ExposeInvocationInterceptor**，第二个元素开始才是通知对象。接着创建代理对象，将拦截器链添加到代理对象中。在执行方法的时候，先递归完所有的通知再执行实际方法。

递归的时候执行around或before方法，递归回去的时候执行after或around方法。
...
其实二级缓存就完全可以解决循环依赖的问题了，但是无法解决使用AOP的时候代理对象的创建。**使用三级缓存是为了让aop的代理对象替换原来的对象**。
