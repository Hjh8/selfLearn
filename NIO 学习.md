...
- 同步或异步。同步是一种可靠的有序运行机制，当我们进行同步操作时，后续的任务是等待当前调用返回，才会进行下一步；而异步则相反，其他任务不需要等待当前调用返回，通常依靠事件、回调等机制来实现任务间次序关系。
- 阻塞与非阻塞。在进行阻塞操作时，当前线程会处于阻塞状态，无法从事其他任务，只有当条件就绪才能继续，比如ServerSocket新连接建立完毕，或数据读取、写入操作完成；而非阻塞则是不管IO操作是否结束，直接返回，相应操作在后台继续处理。
...
I/O 模型：就是用什么样的通道或者通信模式进行数据的传输和接收，很大程度上决定了程序通信的性能。

> IO文件操作需要把数据从 **内核**缓冲区 拷贝到 **用户**缓冲区
