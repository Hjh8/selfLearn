...
> 非阻塞IO不会频繁的阻塞跟唤醒线程（阻塞跟唤醒线程的开销大），而是一直占用CPU。但非阻塞IO有一个非常严重的问题，需要不断地去询问内核数据是否就绪，这样会导致CPU占用率非常高。
...
而epoll是采用**红黑树**存放socket，且每个socket以**双向链表方式关联监听事件**，所以可以一次性就知道是哪个socket就绪。epoll对于select的改进还有：**把 就绪列表 跟 等待列表 分开（数据结构为双向链表）**，如果监听到哪个等待的socket就绪了，把该socket移到就绪列表中，每次将就绪列表返回即可。而select是将全部的socket数组返回（不管是否就绪）。
