...
> 
> `cat awkTest.log | awk -F " " '{print $3}' | sort | uniq -c | sort -nr | awk -F " " '{print $2}' | head -10` 
> 
> - `awk -F " "`：以空格为分隔符
> - `'{print $3}'`：输出第三列
...
> - 使用`uniq -c`命令用于统计并去重重复的行，并输出其出现的次数。但是在使用`uniq  -c`之前，通常需要先对输入进行排序，原因如下: `uniq  -c`命令只能检测连续的重复行。如果输入没有经过排序，那么重复的行可能被分散到不同的位置，无法被`uniq  -c`命令正确统计。通过先对输入进行排序，可以将重复行聚集在一起，确保`uniq  -c`命令能正确统计出重复行的出现次数。
