...
在很多常见下，读操作往往比写操作要更多，如果因为一个资源可能被修改而去加锁，那么一个线程读取这个资源的时候其他线程就不能读取，这样效率会很低。所以出现了读写锁，一个线程在读的时候，其他线程也可以读，但一个线程在写的时候，其他线程就需要阻塞。
...
有任务来的时候，线程池会先判断当前线程是否达到核心线程数，若没达到则创建线程去执行，否则放入工作队列中。当工作队列满的时候，会创建临时线程处理任务，当达到最大线程数的时候会启动拒绝策略。
...
1. state减一。如果state值为0，会将Thread变量设置为null，完成锁的释放。如果当前线程不是最后一个节点的话，则调用 LockSupport.unpark(s.thread); 
