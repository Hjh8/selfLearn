...
AQS是如何实现加锁解锁的？其内部维护了两个属性：state、owerThread、Node。
...
1. 当一个线程使用lock()方法加锁的时候，会使用 <u>CAS操作将state从0变成1</u>。如果修改成功则将 owerThread设置为自己。
2. 如果修改失败说明当前有线程正在使用该锁，此时这个线程会 <u>判断这个owerThread自己</u>，是的话允许加锁，然后state值加一；**这也是可重入锁的实现。** 
