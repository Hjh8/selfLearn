...
- 如果是数字，则直接使用int。
- 否则，使用**简单动态字符串SDS**，对C字符串做了一层封装，多了len和alloc字段。因此**不用遍历整个字符串就可以知道其长度**。采用 **预分配冗余空间** 和 **采用空间懒回收**的方式来减少内存的频繁分配与回收。**二进制安全**，可以存储任意字符，因为使用的是len感知长度，而不是像C使用\0.
...
> set的数据结构：
> 
> - 如果是元素类型是数字且数量不超过512个，则使用intset数组，查找的时候二分查找，但插入的复杂度是O(N)
> 
...
![image-20220728154329377](Redis.assets/image-20220728154329377.png)
...
![image-20220728154651799](Redis.assets/image-20220728154651799.png)
