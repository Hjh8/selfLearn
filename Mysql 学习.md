...
**CHAR(10)**会**固定分配10B**的空间。即若输入数据的长度小于10B，则系统自动在其后添加空格来填满设定好的空间；

而**VARCHAR(10)**的存储长度为**实际数值的长度**，但最多会存储10B。
...
索引相当于书的目录，可以提高sql的查询效率。在InnoDB和MyISAM的引擎下，索引和实际的数据都是存储在磁盘的，只不过进行数据读取的时候会优先把索引加载到内存中。而memory引擎的话数据跟索引都是放在内存中。
...
>
> 1. 所有数据存储在叶子节点，非叶子节点并不存储真正的data，在mysql中非叶子节点存储的是索引
> 2. 为所有叶子结点增加了一个链指针
