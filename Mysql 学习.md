...
红黑树通过维护有序性和自平衡性，实现高效的查找，增加删除操作。例如在C++或java中的map中使用了红黑树作为底层结构。适合在内存中使用。

B树是一种多路搜索树，非叶子节点也存储数据，所以适合于磁盘等外部存储设备上存储大量数据以及需要快速获取数据的情况，常被用于数据库和文件系统中。

B+树，只要叶子节点才存储数据，并且叶子节点之间有指针链接，所以适合于范围查找数据和顺序访问的情况。（跟B树一样，适合在磁盘等外部存储设备上存储大量数据的时候使用）
